...
- **Local-first.** The app runs entirely on the user's machine. No telemetry, no auth, no multi-user, no hosted service. Provider config (CLI paths, Ollama API key) lives encrypted in the SQLite settings table, editable from the UI; a plain `.env` merged with the system environment remains as an optional fallback. Never hardcode paths or values specific to one machine.
